...
import xyz.hollywoodhub.hollywoodhub.rest.gomovies.GetSearchApi;
...
/**
 * Created by rpandey.ppe on 14/09/17.
 */

public class SearchContentListFragment extends ContentListFragment {

    final static String SEARCH_QUERY = "search_query";
    ...
    public static SearchContentListFragment newInstance(String searchQuery) {
        SearchContentListFragment fragment = new SearchContentListFragment();
    ...
    void fetchContents(final int pageNo) {
        showProgress();
        new GetSearchApi(searchQuery, new IRestCallBacks<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if (response != null) {
                    if (response.isSuccessful()) {
                        ...
                        tryFetchSourceFromWebView(searchQuery, call.request().url().toString());
                        return;
                    }
                }
    ...
    void parseContentHTML(String body) {
        ...
        try {
            List<ContentDataModel> contentDataModelList =
                    ParserFactory.getInstance().searchContentList(document);
