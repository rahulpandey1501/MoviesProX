...
import xyz.hollywoodhub.hollywoodhub.rest.gomovies.GetMoviesApi;
...
public class MovieListFragment extends ContentListFragment {
    ...
    public static MovieListFragment newInstance(String categoryId) {
    ...
    void fetchContents(final int pageNo) {
        showProgress();
        new GetMoviesApi(categoryId, pageNo + "", new IRestCallBacks<String>() {
            @Override
    ...
    void parseContentHTML(String body) {
        ...
        try {
            List<ContentDataModel> contentDataModelList =
                    ParserFactory.getInstance().movieContentList(document);
            updateAdapter(contentDataModelList);
        } catch (Exception e) {
            Utils.showToast(Constants.PARSE_FAILED_MESSAGE);
        }
        hideProgress();
        eventBus.postSticky(document); // to get the filter data
    }
